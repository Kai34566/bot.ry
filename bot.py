...
def send_sheriff_menu(chat, sheriff_id, callback_query=None, message_id=None):
    if not is_night:
        if callback_query:
            ...
            bot.answer_callback_query(callback_query.id, "Действия Шерифа доступны только ночью.", show_alert=True)
        return
...
def check_and_transfer_sheriff_role(chat):
    if chat.sheriff_id not in chat.players or chat.players[chat.sheriff_id]['role'] == 'dead':
        ...
        if chat.sergeant_id and chat.sergeant_id in chat.players and chat.players[chat.sergeant_id]['role'] != 'dead':
            ...
            bot.send_message(chat.chat_id, "👮🏼 *Сержант* унаследовал роль 🕵️‍♂️ *Шериф*", parse_mode="Markdown")
        else:
...
def notify_police(chat):
    ...
    if chat.sheriff_id and chat.sheriff_id in chat.players and chat.players[chat.sheriff_id]['role'] == '🕵️‍♂️ Шериф':
        ...
        police_members.append(f"[{sheriff_name}](tg://user?id={chat.sheriff_id}) - 🕵️‍♂️ *Шериф*")
...
def process_deaths(chat, killed_by_mafia, killed_by_sheriff, killed_by_bomber=None, killed_by_maniac=None):
    ...
    if killed_by_sheriff:
        ...
        if victim_id in deaths:
            deaths[victim_id]['roles'].append('🕵️‍♂️ Шериф')
        else:
            deaths[victim_id] = {'victim': victim, 'roles': ['🕵️‍♂️ Шериф']}
    ...
    for victim_id, death_info in deaths.items():
        ...
        if victim['role'] == '💣 Смертник':
            for killer_role in roles_involved:
                if killer_role in ['🤵🏻‍♂️ Дон', '🕵️‍♂️ Шериф', '🔪 Маньяк']:
                    ...
                    if killer_role == '🕵️‍♂️ Шериф' and chat.sheriff_id:
                        sheriff_player_link = f"[{chat.players[chat.sheriff_id]['name']}](tg://user?id={chat.sheriff_id})"
                        combined_message += f"💣 Смертник забрал с собой 🕵️‍♂️ Шерифа {sheriff_player_link}...\n\n"
        ...
        combined_message += f"Говорят, у него в гостях был {role_list}\n\n"
...
def handle_shop_actions(call):
    ...
    if call.data == "shop":
        ...
        shop_text = f"💶 _Баланс_: {escape_markdown(str(profile['euro']))}\n" \
                    ...
                    f"📁 *Поддельные документы* (💶 200)\n_Шериф увидит вас как мирного жителя._"
...
async def game_cycle(chat_id):
    ...
    try:
        while chat.game_running:  # Основной цикл игры
            ...
            killed_by_bomber = None# Жертва шерифа
            ...
            if chat.lawyer_target and chat.sheriff_check and chat.lawyer_target == chat.sheriff_check:
                ...
                bot.send_message(chat.sheriff_check, '🕵️‍♂️ Шериф навестил тебя, но адвокат показал, что ты мирный житель.', parse_mode="Markdown")
                if chat.sergeant_id and chat.sergeant_id in chat.players:
                    sergeant_message = f"🕵️‍♂️ Шериф проверил {checked_player['name']}, его роль - 👨🏼‍🌾 Мирный житель."
                ...
                if chat.sheriff_check and chat.sheriff_check in chat.players:
                    ...
                    if checked_player['fake_docs'] > 0:
                        ...
                        bot.send_message(chat.sheriff_check, '🕵️‍♂️ Шериф навестил тебя, но ты показал фальшивые документы.', parse_mode="Markdown")
                        ...
                        bot.send_message(chat.sheriff_check, '🕵️‍♂️ Шериф решил навестить тебя.', parse_mode="Markdown")
                    if chat.sergeant_id and chat.sergeant_id in chat.players:
                        sergeant_message = f"🕵️‍♂️ Шериф проверил {checked_player['name']}, его роль - {checked_player['role']}."
                        bot.send_message(chat.sergeant_id, sergeant_message)
...
def callback_handler(call):
    ...
    try:
        ...
        # Проверка, что действия шерифа доступны только ночью
        if role == '🕵️‍♂️ Шериф':
            if not is_night:  # Если сейчас не ночь
                bot.answer_callback_query(call.id, text="Действия шерифа доступны только ночью.")
                return

            # Проверка, совершил ли шериф уже действие
        ...
        if call.data.startswith('confirm'):
            ...
            if action in ['ш', 'с', 'м', 'мк', 'д', 'б', 'л', 'а', 'vote']:
                ...
                if player_role == '🕵️‍♂️ Шериф' and action == 'ш':
                    if not is_night:  # Если сейчас не ночь
                        bot.answer_callback_query(call.id, text="Действия шерифа доступны только ночью.")
                        return# Шериф проверяет игрока
                    if chat.players[from_id].get('action_taken', False):  # Проверяем, сделал ли уже шериф действие
                        bot.answer_callback_query(call.id, text="Вы уже сделали выбор этой ночью.")
                        return

                    chat.sheriff_check = target_id
                    chat.players[from_id]['action_taken'] = True  # Отмечаем, что шериф сделал действие
                    ...
                    bot.send_message(chat.chat_id, f"🕵️‍♂️ *Шериф* ушел искать злодеев...", parse_mode="Markdown")
                ...
                elif player_role == '🕵️‍♂️ Шериф' and action == 'с':
                    if not is_night:  # Если сейчас не ночь
                        bot.answer_callback_query(call.id, text="Действия шерифа доступны только ночью.")
                        return
                        # Шериф стреляет в игрока
                    if chat.players[from_id].get('action_taken', False):  # Проверяем, сделал ли уже шериф действие
                        bot.answer_callback_query(call.id, text="Вы уже сделали выбор этой ночью.")
                        return
                    ...
                    bot.send_message(chat.chat_id, f"🕵️‍♂️ *Шериф* взял свой пистолет...", parse_mode="Markdown")
